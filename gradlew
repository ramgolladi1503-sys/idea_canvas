...
# Escape application path for use in -Dapp.name
APP_NAME=`basename "$0"`

# Add application path and name to the environment for others to use.
APP_BASE_NAME="$APP_NAME"
APP_HOME_DIR="$APP_HOME"

# Add the standard JVM options to the command.
JVM_OPTS="$DEFAULT_JVM_OPTS $JAVA_OPTS $GRADLE_OPTS"

# Split up the JVM_OPTS values at spaces so they can be passed as separate arguments.
# This is required to allow "-Dfoo=bar" style JVM_OPTS to be passed in.
JVM_OPTS_ARRAY=()
